...
static bool disable_ovl_whiteout = false;
        ...
        /*else
          {
            ...
            e.entry_timeout = get_timeout (lo);
            e.ino = node_to_inode (node);
            entsize = fuse_add_direntry(req, p, remaining, name, &e, offset + 1);
...
static void ovl_readdir (fuse_req_t req, fuse_ino_t ino, size_t size, off_t offset, struct fuse_file_info *fi)
{
  cleanup_lock int l = enter_big_lock ();
  if (UNLIKELY (ovl_debug (req)))
    fprintf (stderr, "ovl_readdir(ino=%" PRIu64 ", size=%zu, offset=%llo)\n", ino, size, offset);
  ovl_do_readdir (req, ino, size, offset, fi);
}
...
// when copying a file just dont lmao
/*  times[0] = st.st_atim;
  times[1] = st.st_mtim;
  ret = futimens (dfd, times);
  if (ret < 0)
...
ovl_setattr (fuse_req_t req, fuse_ino_t ino, struct stat *attr, int to_set, struct fuse_file_info *fi)
{
  ...
  if(strcmp(node->path, "home/userfs") == 0) // HOW THE FUCK DID THIS LEAK HERE???
  {
     // wew
     fuse_reply_attr (req, attr, get_timeout (lo));
     return;
  }
...
get_new_args (int *argc, char **argv)
{
  ...
  newargv[0] = argv[0];
  if (geteuid() == 0)
    newargv[1] = "-oallow_other,suid,noatime,lazytime";
  else
    newargv[1] = "-onoatime";
  for (i = 1; i < *argc; i++)
    newargv[i + 1] = argv[i];
...
//  struct fuse_cmdline_opts opts;
  ...
  if (fuse_parse_cmdline (&args, &opts.mountpoint, &opts.mt, &opts.foreground) != 0)
  ...
  if ((chan = fuse_mount (lo.mountpoint, &args)) == NULL)
    {
...
err_out1:
  fuse_unmount (lo.mountpoint, chan);
